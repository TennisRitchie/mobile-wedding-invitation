...
const ADMIN_PORT = 4747;
const SAVE_URL = `http://localhost:${ADMIN_PORT}/gallery`;
...
export default function TacocatAdmin() {
  const [items, setItems] = useState<GalleryItem[]>(() =>
    galleryItems.map((i) => ({ ...i }))
  );
  ...
  // 저장 서버가 떠 있는지 확인 (없으면 보기 전용 + JSON 내려받기)
  useEffect(() => {
    let alive = true;
    fetch(SAVE_URL, { method: "GET" })
  ...
  const dirty = useMemo(
    () => JSON.stringify(items) !== JSON.stringify(galleryItems),
    [items]
  );
    ...
    <main className="min-h-screen bg-neutral-50 text-neutral-900">
      <header className="sticky top-0 z-20 border-b border-neutral-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-3 px-5 py-3">
          ...
          <button
            onClick={() => setItems(galleryItems.map((i) => ({ ...i })))}
            disabled={!dirty}
          ...
          <button
            onClick={save}
            disabled={!serverUp || !dirty || status.kind === "saving"}
        ...
        {serverUp === false && (
          <p className="border-t border-amber-200 bg-amber-50 px-5 py-2 text-[12px] text-amber-800">
            저장 서버가 꺼져 있어 <b>보기 전용</b>입니다. 파일로 저장하려면
            터미널에서 <code className="font-mono">npm run admin</code> 으로
            실행하세요. (지금도 &quot;JSON 내려받기&quot;로 받아
            <code className="font-mono"> data/gallery.json</code> 에 덮어쓸 수
            있습니다.)
          </p>
        )}
        {status.kind === "saved" && (
          <p className="border-t border-emerald-200 bg-emerald-50 px-5 py-2 text-[12px] text-emerald-800">
            data/gallery.json 에 저장했습니다 ({status.at}). 청첩장 화면에 바로
            반영됩니다.
          </p>
        )}
        {status.kind === "error" && (
